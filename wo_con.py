...
from utils.optimtools import get_preference_vectors, batch_group_probs, get_d_paretomtl_init, get_d_paretomtl_wo_recloss
...
def train_model(item_groups, user_neighbors):
    ...
    # In this code, we don't consider the recloss constraint, so the optimization step is the same as finding initial solution!
    ...
    for epoch in range(1, args.max_epoch+1):
        ...
        for _, (users, pos_items) in enumerate(train_loader):
            ...
            grads = torch.stack([torch.cat(grads[i]) for i in range(2)])
            losses_vec = torch.stack([rsd_speo.data, rsd_nspneo.data])    # calculate the weights
            weight_vec = get_d_paretomtl_wo_recloss(grads, losses_vec, ref_vec, pref_idx)
        ...
        #################################################    
        All_entro.append(sum(All_entro)/len(total_entro))
        #################################################
